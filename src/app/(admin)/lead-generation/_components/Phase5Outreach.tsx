...
import { Switch } from "@/app/(admin)/lead-generation/_components/ui/switch";
import { Badge } from "@/app/(admin)/lead-generation/_components/ui/badge";
import { Label } from "@/app/(admin)/lead-generation/_components/ui/label";
import { Textarea } from "@/app/(admin)/lead-generation/_components/ui/textarea";
import { PhaseShell } from "./PhaseShell";
import { IncompleteState } from "./IncompleteState";
import { MessageCircle, Mail, Camera, Copy, ExternalLink, Sparkles, Loader2, RefreshCw, FileText } from "lucide-react";
import type { RankedLead, OutreachChannel, OutreachLanguage } from "@/app/(admin)/lead-generation/_lib/types";
...
export function Phase5Outreach({
  ...
  async function generateDraft() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/lead-generation/api/outreach", {
        method: "POST",
  ...
  async function markAsContacted() {
    if (!selected) return;
    try {
      await fetch("/lead-generation/api/crm/leads", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
