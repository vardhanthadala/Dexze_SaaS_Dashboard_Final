...
import { Input } from "@/app/(admin)/lead-generation/_components/ui/input";
...
import type { Lead, ScrapeInput } from "@/app/(admin)/lead-generation/_lib/types";
...
export function Phase1Scrape({
  ...
  async function addManualUrl() {
    ...
    try {
      const res = await fetch("/lead-generation/api/scrape-url", {
        method: "POST",
  ...
  async function runScrape() {
    ...
    setLeads([]);
    try {
      const res = await fetch("/lead-generation/api/scrape", {
        method: "POST",
        headers: { "content-type": "application/json" },
